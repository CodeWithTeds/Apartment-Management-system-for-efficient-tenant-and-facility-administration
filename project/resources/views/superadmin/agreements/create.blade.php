<x-superadmin-layout>
    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5">
        <div class="w-full mb-1">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Create New Agreement</h1>
            </div>
        </div>
    </div>

    <div class="p-4 bg-white rounded-lg shadow">
        <form action="{{ route('superadmin.agreements.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea id="content" name="content" rows="15"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Enter the agreement content here..." required>{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status"
                    ...
                    <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>Pending (Send to Admin)</option>
                </select>
                @error('status')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
