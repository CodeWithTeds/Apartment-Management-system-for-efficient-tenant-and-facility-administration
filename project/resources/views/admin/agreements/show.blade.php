<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto p-8 bg-white shadow-lg border border-gray-200 my-8">
        ...
        <div class="text-center mb-8 border-b-2 border-gray-300 pb-6">
            <h1 class="text-3xl font-bold text-gray-900 mb-2 uppercase tracking-wide">LEASE AGREEMENT</h1>
                ...
                <p>Status: 
                    @switch($agreement->status)
                        @case('draft')
                            ...
                            @break
                        @case('pending')
                            <span class="bg-yellow-100 text-yellow-800 text-xs font-medium px-2 py-1 rounded">Pending Review</span>
                            @break
                        @case('approved')
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-semibold text-gray-900 mb-2">Property Manager (Admin)</h3>
        ...
        <div class="mt-8 pt-6 border-t border-gray-200">
            <div class="flex flex-wrap items-center justify-center space-x-4">
                @if(!$agreement->admin_acknowledged_at && $agreement->status == 'pending')
                <form action="{{ route('admin.agreements.acknowledge', $agreement) }}" method="POST" class="inline">
                    @csrf
                    <div class="mb-4">
                        <label for="admin_notes" class="block mb-2 text-sm font-medium text-gray-900">Your Notes (Optional)</label>
                        <textarea id="admin_notes" name="admin_notes" rows="3" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"></textarea>
                    </div>
                    <button type="submit"
                ...
                <form action="{{ route('admin.agreements.reject', $agreement) }}" method="POST" class="inline">
                    @csrf
                    <div class="mb-4">
                        <label for="admin_notes" class="block mb-2 text-sm font-medium text-gray-900">Rejection Reason (Required)</label>
                        <textarea id="admin_notes" name="admin_notes" rows="3" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required></textarea>
                    </div>
                    <button type="submit"
