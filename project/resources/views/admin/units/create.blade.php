<x-admin-layout>
    <div class="container mx-auto px-4 py-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Add New Unit</h1>
            ...
            <form action="{{ route('admin.units.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="unit_number" id="unit_number" value="{{ old('unit_number') }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('unit_number') border-red-500 @enderror" required>
                        @error('unit_number')
                    ...
                    <div>
                        ...
                        <select name="unit_type" id="unit_type" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <option value="1 Bedroom" {{ old('unit_type') === '1 Bedroom' ? 'selected' : '' }}>1 Bedroom</option>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="rent_price" id="rent_price" value="{{ old('rent_price') }}" step="0.01" min="0" class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('rent_price') border-red-500 @enderror" required>
                        </div>
                        @error('rent_price')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
